...
class EnhanceTestCase(unittest.TestCase):
    ...
    def test_left_enhancement_vectors(self):
        ...
        alphaL,alphaR,betaL,betaR = enhance.left_enhancement_vectors(Alinv,Arinv,solution_order,modes,base.psi)
        
        # Make sure both left and right enhancement vectors are correct
        npt.assert_array_almost_equal(alphaL, np.array([0.7734375,  0.6796875,  0.4375]), decimal = 7)
        npt.assert_array_almost_equal(alphaR, np.array([0.2265625, -0.1328125,  0.    ]), decimal = 7)
        npt.assert_array_almost_equal(betaL , np.array([0.2265625,  0.1328125,  0.    ]), decimal = 7)
        npt.assert_array_almost_equal(betaR , np.array([0.7734375, -0.6796875,  0.4375]), decimal = 7)
    ...
    def test_face_value(self):
        ...
        u = np.arange(1,(order+1)*5+1).reshape((order+1,5),order='F')
